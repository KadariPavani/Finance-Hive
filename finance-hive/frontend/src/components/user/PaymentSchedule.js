...
const PaymentSchedule = ({ loanId }) => {
  ...
  const handlePaymentConfirmation = async () => {
    ...
    try {
      // Simulate fetching transaction ID after payment completion
      const simulatedTransactionId = `TXN${Math.floor(Math.random() * 1000000000)}`; // Simulate unique transaction ID

      // Update the frontend payment data with the new status and transaction ID
      ...
      // Update backend with the transaction ID and screenshot
      await axios.put(`http://localhost:5000/api/loans/payment/${loanId}/${currentPayment.sno}`, {
        transactionId: simulatedTransactionId,  // Send transaction ID to backend
        screenshot, // Send screenshot to backend for processing
      });
