...
import CompletionPage from '../UserCompletionPage/CompletionPage';
...
const PersonalDetailsForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (formData[key] !== null && formData[key] !== '') {  // Ensure data is not null or empty
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/personal-details/submit', {
        method: 'POST',
        body: formDataToSend,
      });
      ...
      // Handle successful form submission (response from the server)
      const responseData = await response.json();
      console.log('Form successfully submitted:', responseData);

      // Transition to the CompletionPage
      setIsCompleted(true);
    } catch (error) {
  ...
  return (
    <div className="personal-details-app-container" id="money-matters">
